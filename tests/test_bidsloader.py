...
class TestBIDSLoader(unittest.TestCase):
    ...
    def test_BIDSLoader_init_dataisderivatives_blank(self):
        test_dir = os.path.dirname(__file__)
        root_dir = os.path.join(test_dir, 'bids_sample/train')
        data_ent = [{'subject': '123', 'session': '5'},
        ...
        bdc = BIDSLoader(
            ...
            target_entities=target_ent,
            batch_size=batch_size)
        self.assertEqual(len(bdc.data_derivatives_names),
                         len(bdc.data_entities))
    ...
    def test_BIDSLoader_init_targetisderivatives_blank(self):
        ...
        self.assertEqual(len(bdc.target_derivatives_names),
                         len(bdc.target_entities))
        return
    ...
    def test_getmatchingimages_matching_ents(self):
        ...
        self.assertEqual(
            ses_im.get_entities()['session'],
            image_to_match.get_entities()['session'])
        return
