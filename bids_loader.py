...
class BIDSLoader():
    '''
           ...
           data_entities : list [dict]
               ...
               For example: ({'subject': '', 'session': '1', 'desc': 'Normalized'}, {'session': '1', 'desc': 'defaced'}) would return
               samples of two images: The first could be 'sub-123_ses-1_desc-Normalized.nii.gz', and the second would be
           ...
           root_list : list
               Reserved. Not yet implemented. List of BIDS root directories, if data must be loaded from different BIDS directories.
               There must be exactly len(data_entities) + len(target_entities) entries in the list, with the order
               corresponding to the order of the data_entities, followed by the target_entities.
    ...
    def get_matching_images(image_to_match: BIDSImageFile,
        ...
        for idx, potential_im in enumerate(potential_matches):
            ...
            for entity, value in ents_to_match.items():
                if(entity not in potential_im_ents.keys() or value != potential_im_ents[entity]):
                    continue
            else:
